...
async function saveReport(request: Request, env: any, subscriptionId: string): Promise<Response> {
  ...
  if (manual) {
    ...
    if (manual.reply_token) {
      try {
        await line(env, 'reply', { replyToken: manual.reply_token, messages: [{ type: 'text', text: resultText }] });
      } catch (err) {
        console.warn('[saveReport] Failed to reply with token, falling back to push:', err);
...
async function command(env: any, userId: string, value: string, replyToken: string | null): Promise<string | null> {
  ...
  if (value.startsWith('立即檢查 ')) {
    ...
    await env.DB.prepare('INSERT INTO manual_check_requests (id,subscription_id,line_user_id,reply_token) VALUES (?,?,?,?)').bind(crypto.randomUUID(), subscriptionId, userId, replyToken).run();
    return null;
  }
  return '輸入「說明」查看指令。';
}
    ...
    try {
      ...
      if (request.method === 'POST' && failMatch) {
        ...
        if (item) {
          ...
          if (item.reply_token) {
            try {
              await line(env, 'reply', { replyToken: item.reply_token, messages: [{ type: 'text', text: failText }] });
            } catch (err) {
              console.warn('[fail] Failed to reply with token, falling back to push:', err);
